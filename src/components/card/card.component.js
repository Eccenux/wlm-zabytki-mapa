...
function controller(
  ...
  vm.artType = () => {
    ...
    if (vm.data.tags) {
      const tag =
        vm.data.tags.historic || vm.data.tags.man_made || vm.data.tags.tourism;
  ...
  function getNatureUploadUrl() {
    ...
    const description = `${type[0].toUpperCase()}${type.substring(1)}: ${
      vm.data.name
    }`;
    ...
    url += `&description=${description}&categories=${category}&id=${vm.data.id}`;
    url += `&lat=${dataService.getLastCoord().lat}&lon=${
      dataService.getLastCoord().lng
    }`;
    return url;
  }
